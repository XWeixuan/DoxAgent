...
def _copy_frozen_inputs(
    ...
    *,
    limit: int,
) -> tuple[SQLiteCDECRRegistry, list[str], dict[str, str]]:
    ...
    required_message_ids = {
        mention.message_id for mention in selected_mentions.values()
    }
    ...
    return output, [item.message_id for item in selected_sources], original_versions
...
def _package_invariants(registry: SQLiteCDECRRegistry) -> dict[str, object]:
    ...
    embedding_by_package = {
        item.owner_id: item for item in package_embeddings
    }
    ...
    for candidate in registry.list_package_external_relation_candidates():
        ...
        if (
            memberships
            and memberships[0].package_id
            == registry.resolve_package_root(candidate.target_package_id)
        ):
    ...
    return {
        ...
        "membership_count": sum(
            len(registry.list_memberships_for_package(item.package_id))
        ...
        "packages_without_embedding": sorted(
            ...
            - {item.owner_id for item in package_embeddings}
        ),
        "package_embedding_hash_mismatches": sorted(
            embedding_hash_mismatches
        ),
        "external_candidate_count": len(
            registry.list_package_external_relation_candidates()
        ),
        "self_external_candidate_count": self_external_candidates,
        "formal_external_relation_count": len(
            registry.list_package_external_relations()
        ),
    }
...
def main() -> int:
    ...
    registry, message_ids, original_versions = _copy_frozen_inputs(
    ...
    before_mentions = {
        mention.mention_id: hashlib.sha256(
            mention.model_dump_json().encode("utf-8")
        ).hexdigest()
    ...
    for index, message_id in enumerate(message_ids, start=1):
        ...
        processing_key = f"package-n11-n13-acceptance:{message_id}"
        ...
        try:
            canonical_fields.resolve_package_hints(
            ...
            packages = engine._correct_packages_v13(
                packages, models, run_id=run_id
            )
    ...
    report = {
        "report_version": "cdecr-package-n11-n13-real-acceptance-v1",
        ...
        "failed_document_count": len(message_ids) - succeeded,
