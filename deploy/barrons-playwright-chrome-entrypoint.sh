...
if [ "$(id -u)" = 0 ]; then
  ...
  exec setpriv --reuid="$service_uid" --regid="$service_gid" --init-groups \
    env HOME=/home/chromecontrol LANG=en_US.UTF-8 LC_ALL=en_US.UTF-8 \
        ...
        /usr/local/bin/barrons-playwright-chrome-entrypoint "$@"
fi

if [ "$(id -u)" != "$service_uid" ]; then
  echo "Playwright Chrome control must run as uid ${service_uid}" >&2
  exit 1
fi

exec 9>"$profile_dir/.control.lock"
if ! flock -n 9; then
  echo "The isolated Playwright Chrome Profile already has a writer" >&2
  exit 1
fi
...
/opt/playwright-venv/bin/python \
  /usr/local/bin/barrons-playwright-chrome-control.py &
runner_pid=$!
wait "$runner_pid"
status=$?
runner_pid=
