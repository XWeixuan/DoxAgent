#!/bin/bash
set -euo pipefail
cd "${DOXAGENT_PROJECT_ROOT:-$(cd -- "$(dirname -- "$0")/.." && pwd)}"
...
install -m 600 "${DOXAGENT_CODEX_AUTH_SOURCE:?Set DOXAGENT_CODEX_AUTH_SOURCE to the authorized auth.json}" "$volume/codex-home/auth.json"
