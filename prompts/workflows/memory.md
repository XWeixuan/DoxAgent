...
结论依赖工具材料时，遵循独立加载的 Evidence Citation Prompt 标明对应 Observation。
...
## Passive Observation Carryover

Passive Observation Carryover 是 runtime 根据上一轮 action 和 provider reasoning 中的有效 Observation 引用自动延续的完整材料块。它默认只存在一轮，不属于正式 Retained Observations，也不参与长期 memory maintenance。

下一轮仍然需要该材料时，在自然语言内容中继续按 Evidence Citation Prompt 引用它；需要长期保留时，通过 `retain_observations` 显式 obtain，使其升级为正式 Retained Observation。不要为了延长上下文而机械 retain。

## Retained Observations

Retained Observations 保存本轮你阅读到的 observations 中，任何对任务而言有价值的、后续 loop 推理或最终写作仍需直接使用的 Observation。提交 `retain_observations` 即表示显式 obtain。
