...
class Views:
    ...
    def create(
        ...
        with self.store.connect(write=True) as db:
            ...
            rows = db.execute(
                "SELECT ticker,payload FROM object_current WHERE kind='ticker' "
                "AND valid_to IS NULL AND (? IS NULL OR ticker=?) "
                "ORDER BY ticker",
                (ticker, ticker),
            ).fetchall()
            ...
            first = db.execute("SELECT value FROM read_meta WHERE key='first_at'").fetchone()[0]
            ...
            if (
                ...
                maintenance = bool(
                    db.execute(
                        "SELECT 1 FROM object_current WHERE kind='native:runtime_tasks' AND ticker IN "
            ...
            snapshot = {
                ...
                "capture_proofs": {row[0]: json.loads(row[1]) for row in db.execute("SELECT source,payload FROM capture_proofs")},
            }
            identity = self.store.save_token(
                owner, self.scope(page, ticker), snapshot, view=True, now=now, connection=db
            )
    ...
    def get(self, owner: str, identity: str, ticker: str | None = None) -> dict[str, Any]:
        from doxagent.v2_read.query_budget import request_views
        cache = request_views.get()
        key = (str(self.store.path), owner, identity)
        try:
            value = cache[key] if cache is not None and key in cache else self.store.token(identity, owner, view=True)
        ...
        from doxagent.v2_read.query_budget import frozen_proofs
        if "capture_proofs" in value:
            frozen_proofs.set((value["seq"], value["capture_proofs"]))
        else:
            frozen_proofs.set((value["seq"], None))
        return value
