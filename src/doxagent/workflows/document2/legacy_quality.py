...
class Document2LegacyQualityMixin:
    ...
    def _resolve_blockers(
        ...
        while unresolved_objections:
            repair_tasks = self._document2_field_repair_tasks(
                checkpoint, unresolved_objections, stalled_task_ids=stalled_task_ids
            )
            if not repair_tasks:
                break
            task_index += 1
            ...
            self._assert_no_proposed_patches(
                result,
                node,
                "O1 resolver must return Document2FieldRepairResult with a typed field update, or one complete revised_candidate only for cross_field tasks.",
            )
            ...
            repair_result, boundary_notes = self._canonicalize_document2_field_repair_decisions(
                repair_result, repair_task
            )
            audit = self._apply_document2_field_repair_transaction(
                checkpoint, repair_result, boundary_notes=boundary_notes
            )
    ...
    def _validate_document2_field_repair_result_matches_task(
        self, result: Document2FieldRepairResult, task: Document2FieldRepairTask
    ) -> None:
    ...
    def _canonicalize_document2_field_repair_decisions(
        ...
        if dropped_target_count:
            notes.append(
                f"Dropped {dropped_target_count} task-external target_finding_ids from O1 field repair output."
            )
        dropped_unresolved_count = len(result.unresolved_finding_ids) - len(unresolved_finding_ids)
        if dropped_unresolved_count:
            notes.append(
                f"Dropped {dropped_unresolved_count} task-external unresolved_finding_ids from O1 field repair output."
            )
        changed = False
        ...
        if dropped_decision_count:
            notes.append(
                f"Dropped {dropped_decision_count} task-external decision records from O1 field repair output."
            )
        ...
        *,
        ...
        try:
            revision = document2_revision_from_field_repair_result(
                result, before_patch=before_patch
            )
        ...
        for decision in result.decisions:
            ...
            if self._document2_resolution_decision_retains_blocker(
                checkpoint, decision, revalidation_findings=revalidation_findings
            ):
    ...
    def _apply_document2_resolution_transaction(
        self, checkpoint: WorkflowCheckpoint, plan: Document2ResolutionPlan
    ) -> Document2TransactionAudit:
    ...
    def _pending_expectation_patch_for_field_repair_result(
        self, checkpoint: WorkflowCheckpoint, result: Document2FieldRepairResult
    ) -> BlackboardPatch | None:
        for patch in checkpoint.pending_patches:
            if (
                patch.target.document_type is DocumentType.EXPECTATION_UNIT
    ...
    def _replace_pending_patch_from_transaction(
        self, checkpoint: WorkflowCheckpoint, revision_patch: BlackboardPatch
    ...
    def _record_document2_transaction_revision(
        ...
        existing = next(
            (item for item in entries if item.get("expectation_id") == revision.expectation_id),
            None,
        )
    ...
    def _merge_document2_review_findings_metadata(
        self, checkpoint: WorkflowCheckpoint, findings: list[Document2ReviewFinding]
    ) -> None:
        if not findings:
            return
    ...
    def _bridge_document2_blocking_findings_to_objections(
        ...
        for finding in findings:
            ...
            bridged.append(
                finding.model_copy(
                    update={"source_objection_id": objection.objection_id}, deep=True
                )
            )
        return bridged

    def _document2_objection_from_review_finding(
        self, checkpoint: WorkflowCheckpoint, finding: Document2ReviewFinding
    ) -> Objection:
        return Objection(
    ...
    def _apply_document2_objection_transition(
        self, checkpoint: WorkflowCheckpoint, decision: Document2ResolutionDecisionRecord
    ) -> None:
        if decision.objection_id is None:
            return
        changed_paths = self._localized_changed_paths(decision.changed_paths)
        note = self._objection_resolution_note_text(
            decision.resolution_note, decision=decision.decision
        )
        if decision.decision == "resolved":
            self.blackboard.resolve_objection(
                checkpoint.run_id, decision.objection_id, note, changed_paths=changed_paths
            )
    ...
    def _document2_field_repair_tasks(
        ...
        *,
        ...
        findings, metadata_changed = self._active_document2_blocking_findings(
            checkpoint, unresolved_objections
        )
        if metadata_changed:
            self._merge_document2_review_findings_metadata(checkpoint, findings)
        ...
        for objection in unresolved_objections:
            ...
            for routed_finding in self._routed_document2_repair_findings(
                ...
                field_family = self._document2_field_family_for_paths(
                    target_paths, primary_path=routed_finding.target_path
                )
        ...
        return [
            ...
            and any((objection_id in unresolved_by_id for objection_id in task.objection_ids))
        ]

    def _active_document2_blocking_findings(
        self, checkpoint: WorkflowCheckpoint, unresolved_objections: list[Objection]
        ...
        unresolved_ids.update(
            (finding.source_objection_id for finding in bridged if finding.source_objection_id)
        )
        return (
            [
                finding
    ...
    def _is_numeric_sanity_review_finding(self, finding: Document2ReviewFinding) -> bool:
        ...
        return any(
            (
                "deterministic_numeric_sanity" in item or "numeric_sanity" in item
                for item in finding.supplemental_context
            )
        )

    def _document2_field_repair_task_from_group(
        self, checkpoint: WorkflowCheckpoint, group: dict[str, Any]
    ) -> Document2FieldRepairTask:
        ...
        return Document2FieldRepairTask(
            ...
            allowed_output_contract=self._allowed_output_contract_for_field_family(field_family),
        )
    ...
    def _current_document2_candidate(
        self, checkpoint: WorkflowCheckpoint, expectation_id: str
    ...
    def _routed_document2_repair_findings(
        self, checkpoint: WorkflowCheckpoint, objection: Objection, finding: Document2ReviewFinding
    ) -> list[Document2ReviewFinding]:
        ...
        for expectation_id in expectation_ids:
            routed_finding = finding.model_copy(
                update={"expectation_id": expectation_id}, deep=True
            )
            child = self._ensure_document2_candidate_routing_objection(
                checkpoint, objection, routed_finding
            )
            child_objection_ids.append(child.objection_id)
            routed.append(
                routed_finding.model_copy(
                    update={"source_objection_id": child.objection_id}, deep=True
                )
            )
    ...
    def _attributed_document2_expectation_ids(
        ...
        target_resolved = self._resolve_document2_expectation_refs(
            [target.expectation_id], candidate_ids=candidate_ids, document_ids=document_ids
        )
        document_resolved = self._resolve_document2_expectation_refs(
            [target.document_id], candidate_ids=candidate_ids, document_ids=document_ids
        )
        finding_resolved = self._resolve_document2_expectation_refs(
            [finding.expectation_id], candidate_ids=candidate_ids, document_ids=document_ids
        )
        path_resolved = self._resolve_document2_expectation_refs(
            [objection.target_path, target.field_path, finding.target_path, *finding.target_paths],
        ...
        primary_union = self._dedupe_strings(
            (expectation_id for resolved in primary_sets for expectation_id in resolved)
        )
    ...
    def _resolve_document2_expectation_refs(
        self, refs: Iterable[Any], *, candidate_ids: list[str], document_ids: dict[str, str]
    ) -> list[str]:
        resolved: list[str] = []
    ...
    def _document2_subject_requests_all_pending_candidates(
        self, objection: Objection, finding: Document2ReviewFinding
    ) -> bool:
        paths = [
    ...
    def _drop_document2_unroutable_objection(
        ...
        *,
        ...
        self.blackboard.reject_objection(
            checkpoint.run_id, objection.objection_id, note, changed_paths=["document2.routing"]
        )
        self.blackboard.add_working_memory_entry(
            checkpoint.run_id,
            author_agent=AgentName.SYSTEM,
    ...
    def _ensure_document2_candidate_routing_objection(
        ...
        return self.blackboard.create_objection(
            ...
            Objection(
                ...
                target_path=finding.target_path,
                dedupe_hash=f"document2_candidate_routing:{source_objection.objection_id}:{finding.expectation_id}",
                status=ObjectionStatus.OPEN,
            ),
        )

    def _resolve_document2_split_routing_objection(
        self, checkpoint: WorkflowCheckpoint, objection: Objection, child_objection_ids: list[str]
    ) -> None:
        if not objection.is_unresolved:
            return
    ...
    def _document2_field_family_for_paths(
        self, target_paths: list[str], *, primary_path: str
    ) -> str:
    ...
    def _allowed_output_contract_for_field_family(self, field_family: str) -> dict[str, Any]:
        decision_rules = [
            "decisions is the only decision source and must contain exactly one item per routed finding_id.",
            "Do not output top-level decision, target_finding_ids, unresolved_finding_ids, or per-item objection_id. Runtime derives them.",
        ]
        non_revision_rules = [
            "When every decision is resolved, rejected, or deferred, do not return typed field updates and do not return revised_candidate.",
            "For deferred decisions, provide unresolved_reason.",
        ]
        common_rules = [
            "Do not return patches, changes, path_map, JSON Patch operations, or multiple candidates.",
            "O1 may propose a repair; transaction and deterministic revalidation decide blocker closure.",
            *decision_rules,
            *non_revision_rules,
        ]
        if field_family == "cross_field":
            return {
                ...
                "rules": [
                    "When any decision is accepted or partially_accepted, return exactly one complete candidate business body as revised_candidate.",
                    "Do not return typed partial field updates for cross_field tasks.",
                    "Omit document_id, document_type, ticker, created_at, and updated_at; runtime preserves them.",
                    *common_rules,
                ],
            }
        output_field = field_family
        field_specific_rules = [
            f"When decision is accepted or partially_accepted, return exactly one complete replacement value in the top-level '{output_field}' field.",
        ...
        return {
            ...
            "schema_notes": {
                "RealizedFact_fields": ["event_id", "description", "price_reaction"],
    ...
    def _field_repair_context(
        ...
        *,
        ...
        output_guidance = [
            ...
            "Return one decisions item per routed finding_id; decisions is the only decision source.",
            "Do not output top-level decision, target_finding_ids, unresolved_finding_ids, or per-item objection_id.",
            "Do not merge conflicting reviewer opinions into one summary finding.",
            "Do not call external tools in this resolver task.",
            "A blocker closes only after transaction acceptance and deterministic revalidation.",
            "For resolved, rejected, or deferred decisions, do not output typed field updates and do not output revised_candidate.",
            "For deferred decisions, provide unresolved_reason.",
        ]
        if task.field_family == "cross_field":
            output_guidance.extend(
                [
                    "For accepted or partially_accepted cross_field tasks, output exactly one complete revised_candidate business body and no typed partial updates.",
                    "Omit document_id, document_type, ticker, created_at, and updated_at; runtime preserves them.",
                ]
            )
        else:
            ...
            output_guidance.extend(
                [
                    f"For accepted or partially_accepted single-field repair, do not output revised_candidate; return exactly one complete replacement value in the top-level {output_field} field."
                ]
            )
        return {
            ...
            "resolution_request": "Resolve this Document2 field repair task. The resolver selected the field_family and target_paths; do not expand scope.",
    ...
    def _objection_resolution_context(
        ...
        *,
        ...
        relevant_patches = self._objection_resolution_relevant_patches(
            checkpoint.pending_patches, unresolved_objections
        )
        output_guidance = [
            "Only resolve the objections present in unresolved_objections for this batch. Every listed objection_id must appear exactly once in Document2ResolutionPlan.decisions.",
            "When duplicate_objection_clusters contains ids from this batch, resolve same-cluster objections with a consistent decision and do not leave duplicate siblings open.",
            "Use decision='resolved' when the objection can be closed by the current business content.",
            "Do not call external tools in this resolver task.",
            "Use decision='rejected' only with explicit rationale support.",
            "Use decision='accepted' or 'partially_accepted' only when also returning one complete revised_candidate for the affected expectation_id.",
            "Never return BlackboardPatch or proposed_patches in this resolution batch.",
            "Never return patches, changes, path maps, partial updates, list-wrapped revised_candidate, or multiple revised candidates.",
            "Each non-deferred resolution must include changed_paths.",
        ]
        return {
            ...
            "global_research_context": {
                ...
                "reason": "Full GlobalResearch text was already reviewed upstream; this node uses compact expectation and objection summaries to avoid replaying large context into the resolver.",
            },
            "pending_patches": [
    ...
    def _objection_resolution_relevant_patches(
        self, patches: list[BlackboardPatch], unresolved_objections: list[Objection]
    ) -> list[BlackboardPatch]:
        expectation_patches = [
            patch
        ...
        relevant = [
            patch for patch in expectation_patches if patch.target.expectation_id in target_ids
        ]
    ...
    def _current_numeric_sanity_violation_summary(
        self, checkpoint: WorkflowCheckpoint, objection: Objection
    ) -> dict[str, Any] | None:
        ...
        for patch in checkpoint.pending_patches:
            ...
            for current in self._numeric_sanity_objections_for_patch(checkpoint.ticker, patch):
                ...
                return {
                    ...
                    "current_reason": self._compact_context_text(current.reason, limit=2200),
                }
        return None
    ...
    def _reopen_numeric_sanity_objections_after_o1_revision(
        ...
        existing = (
            getter(
                checkpoint.run_id, [objection.objection_id for objection in revalidation_objections]
            )
            if callable(getter)
            else []
        )
        ...
        for objection in revalidation_objections:
            ...
            if existing is not None and (not existing.is_unresolved):
                self.blackboard.mark_objection_unresolved(
                    checkpoint.run_id,
                    objection.objection_id,
                    "Numeric sanity revalidation failed after O1 revision: revised expectation still contains precise numeric claims without source-appropriate evidence. Narrative-only or unverified labelling is not sufficient; remove the false precision or add market/fundamental evidence.",
                )

    def _next_objection_resolution_batch(
        self, unresolved_objections: list[Objection]
    ) -> list[Objection]:
            ...
            summaries.append(
                {
                    ...
                    "objection_ids": [
                        item.objection_id for item in items[:_OBJECTION_RESOLUTION_BATCH_SIZE]
                    ],
                    "omitted_objection_count": max(
                        0, len(items) - _OBJECTION_RESOLUTION_BATCH_SIZE
                    ),
    ...
    def _pending_expectation_patch_summary(self, patch: BlackboardPatch) -> dict[str, Any]:
        ...
        return {
            ...
            "expectation_name": self._compact_context_text(
                after.get("expectation_name"), limit=180
            ),
    ...
    def _compact_pending_expectation_patch(self, patch: BlackboardPatch) -> dict[str, Any]:
        ...
        return {
            ...
            "expectation_id": after.get("expectation_id") or patch.target.expectation_id,
            "expectation_name": self._compact_context_text(
                after.get("expectation_name"), limit=160
            ),
            "direction": after.get("direction"),
            "why_it_matters": self._compact_context_text(after.get("why_it_matters"), limit=260),
            "market_view": {
            ...
            "realized_facts_summary": self._compact_context_text(
                after.get("realized_facts_summary"), limit=260
            ),
            "realized_facts": [
            ...
            "event_monitoring_direction": {
                "known_event_notice": self._compact_context_text(
                    monitoring.get("known_event_notice"), limit=220
                ),
                "positive_events": [
    ...
    def _field_review_pending_patch_context(
        self, agent_name: AgentName, patches: list[BlackboardPatch]
    ) -> list[dict[str, Any]]:
        expectation_patches = [
            patch
            for patch in patches
        ...
        return {
            ...
            "market_view": {
                ...
                "price_reflection_text": self._compact_context_text(
                    market_view.get("text"), limit=420
                ),
            },
            "realized_facts_price_reactions": [
                self._market_trace_fact_context_summary(item) for item in facts[:6]
            ],
            "realized_facts_summary": self._compact_context_text(
                after.get("realized_facts_summary"), limit=260
            ),
    ...
    def _market_trace_fact_context_summary(self, value: Any) -> dict[str, Any]:
        ...
        return {
            ...
            "when": item.get("when"),
            "pricing_status": item.get("pricing_status") or item.get("pricing_assessment"),
            "price_reaction": {
                "price_change": self._compact_context_text(
                    price_reaction.get("price_change"), limit=180
                ),
                "price_pattern": self._compact_context_text(
                    price_reaction.get("price_pattern"), limit=180
                ),
    ...
    def _field_review_global_research_context(
        self, checkpoint: WorkflowCheckpoint, agent_name: AgentName
    ) -> dict[str, Any]:
    ...
    def _field_review_section_context(
        self, section: ResearchSection, ticker: str
    ) -> dict[str, Any]:
    ...
    def _realized_fact_context_summary(self, value: Any) -> dict[str, Any]:
        ...
        return {
            ...
            "price_reaction": {
                "price_change": self._compact_context_text(
                    price_reaction.get("price_change"), limit=160
                ),
                "price_pattern": self._compact_context_text(
                    price_reaction.get("price_pattern"), limit=160
                ),
                "interpretation": self._compact_context_text(
    ...
    def _variable_context_summary(self, value: Any) -> dict[str, Any]:
        ...
        return {
            ...
            "current_status": self._compact_context_text(item.get("current_status"), limit=320),
    ...
    def _validate_a2_retrieval_quality(
        self, retrieval: DelegatedRetrievalResult, result: AgentResult
    ) -> bool:
        ...
        actual_tools = {
            item.tool_name
            for item in [*result.tool_calls, *retrieval.tool_calls]
    ...
    def _complete_o1_revision_delegations(
        self, checkpoint: WorkflowCheckpoint, result: AgentResult | None = None
    ) -> None:
        if self._document2_actionable_unresolved_objections(
            self.blackboard.list_unresolved_objections(checkpoint.run_id)
        ):
        ...
        for delegation in self.blackboard.list_blocking_delegations(
            checkpoint.run_id, target_agent=AgentName.O1_EXPECTATION_OWNER
        ):
            self.blackboard.complete_delegation(
                checkpoint.run_id, delegation.delegation_id, summary
            )
    ...
    def _localized_changed_path(self, path: str) -> str:
        ...
        return re.sub(
            "\\((?P<action>removed|added|populated with|replaced) (?P<detail>[^)]+)\\)",
            replace,
            text,
        )
