...
class Document2LegacyPromotionMixin:
    ...
    def _promote_document2_candidate_read_only(
        ...
        if blockers:
            audit = document2_promotion_audit(
                candidate, patch=None, status="rejected", blockers=blockers
            )
        ...
        self._submit_document2_promotion_patch(
            checkpoint, promotion_patch, "提升已通过复核的 expectation unit。"
        )
        ...
        if unresolved_objections:
            blockers.append(
                Document2PromotionBlocker(
                    ...
                    reason="Promotion requires all blocking objections to be resolved before the read-only gate.",
                )
            )
        ...
        if blocking_delegations:
            blockers.append(
                Document2PromotionBlocker(
                    blocker_type="candidate_not_ready",
                    target_path="document",
                    reason="Promotion requires all blocking delegations to be completed before the read-only gate.",
                )
            )
        return blockers
    ...
    def _submit_document2_promotion_patch(
        ...
        if patch.author_agent is AgentName.SYSTEM:
            ...
            permissions = AgentPermissions(
                writable_targets=[DocumentType.EXPECTATION_UNIT.value], can_propose_patch=True
            )
        else:
            permissions = self.registry.get(patch.author_agent).runtime.to_permissions()
        self.blackboard.submit_patch(
            checkpoint.run_id, patch, permissions=permissions, trigger_reason=trigger_reason
        )
    ...
    def _record_document2_promotion_audit(
        ...
        checkpoint.metadata = checkpoint.metadata | {
            DOCUMENT2_PROMOTION_AUDITS_KEY: [*existing, audit.model_dump(mode="json")]
        }
