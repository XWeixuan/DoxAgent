...
class Document2LegacyPipelineMixin:
    def _assert_no_proposed_patches(
        self, result: AgentResult, node: WorkflowNode, message: str
    ) -> None:
    ...
    def _accepted_document2_reviewer_result(
        ...
        *,
        ...
        accepted, warnings = sanitize_document2_reviewer_result(
            result, required_output_schema=expected_schema
        )
            ...
            "tool_requirements": [
                {
                    ...
                    "required": True,
                    "purpose": "Required DoxAtlas narrative material for expectation-unit construction.",
    ...
    def _review_expectation_construction(
        self, checkpoint: WorkflowCheckpoint, node: WorkflowNode
    ) -> WorkflowCheckpoint:
        ...
        result = self._run_agent(
            ...
            extra_context={
                ...
                "doxatlas_scope_guardrails": {
                    "doxa_query_propositions": "requires run_id+narrative_code+event_code, narrative_id+event_code, narrative_event_id, or proposition_id; ticker and bare narrative_code are invalid",
                    "doxa_get_ignored_propositions": "requires run_id, run_id+narrative_code, run_id+narrative_code+event_code, narrative_id, or narrative_event_id; ticker and bare narrative_code are invalid",
                    "non_retryable_scope_error_policy": "after a non-retryable scope validation error, finalize from available narrative/analysis evidence with explicit data gaps instead of exhausting ReAct steps",
                },
                "tool_requirements": [
                    {
                        "tool_name": tool_name,
        ...
        accepted_result = self._accepted_document2_reviewer_result(
            checkpoint, node, result, expected_schema="DoxAtlasAuditResult"
        )
        ...
        self._assert_no_proposed_patches(
            result, node, "A1 construction review may raise objections or delegations only."
        )
    ...
    def _resolve_expectation_construction(
        ...
        for delegation in self.blackboard.list_blocking_delegations(
            checkpoint.run_id, target_agent=AgentName.A2_FACT_CHECK
        ):
        ...
        if not unresolved:
            return self._mark_completed(
                checkpoint, node, metadata=self._agent_metadata(node, results) if results else None
            )
        shells = self._expectation_shells_from_checkpoint(checkpoint)
        if not shells:
            raise WorkflowContractError(
        ...
        result = self._run_agent(
            ...
            extra_context={
                "resolution_request": "Resolve A1 construction-review objections by revising expectation shells only. Return ExpectationShellConstructionResult. Do not return BlackboardPatch, proposed_patches, full expectation_unit documents, realized_facts, key_variables, or event monitoring fields.",
                ...
                "tool_requirements": [
                    {
                        ...
                        "gap_policy": "If unavailable, revise shells using current context and list the missing DoxAtlas narrative evidence in unknowns.",
                    }
                ],
            },
        )
        ...
        audit = self._apply_document2_construction_resolution_transaction(
            checkpoint, previous_shells=shells, revised=revised, unresolved_objections=unresolved
        )
        results.append(result)
        return self._mark_completed(
            checkpoint,
            ...
            | {
                "expectation_shells": [shell.model_dump(mode="json") for shell in revised.shells],
                DOCUMENT2_CONSTRUCTION_TRANSACTION_AUDITS_KEY: [audit.model_dump(mode="json")],
            },
        )
    ...
    def _record_document2_construction_transaction_audit(
        self, checkpoint: WorkflowCheckpoint, audit: Document2TransactionAudit
    ) -> None:
        self.blackboard.add_working_memory_entry(
            checkpoint.run_id,
            author_agent=AgentName.SYSTEM,
            content_type="document2_construction_transaction_audit",
            payload={"status": audit.status, "audit": audit.model_dump(mode="json")},
        )
    ...
    def _generate_expectation_details(
        ...
        for order, shell in enumerate(shells):
            ...
            cached = self._cached_workflow_agent_result(
                current, node, AgentName.O1_EXPECTATION_OWNER, cache_key=cache_key
            )
            if cached is not None:
                cached_results[order] = cached
                current = self._record_expectation_detail_status(
                    current, node, order, shell, cache_key=cache_key, status="cached_completed"
                )
                continue
            ...
            current = self._record_expectation_detail_status(
                current, node, order, shell, cache_key=cache_key, status="running"
            )
            jobs.append(
                _ParallelAgentJob(
                    order=order,
            ...
            try:
                ...
                if not cached:
                    self._write_working_memory(
                        current, result, "expectation_detail_candidate_result"
                    )
                self._validate_o1_narrative_tool_gap(result, node)
                candidate = self._expectation_unit_candidate_from_detail_result(
                    current.ticker, shell, result
                )
                ...
                self._validate_patch_contract(
                    legacy_patch, WorkflowNode.GENERATE_EXPECTATION_DETAILS
                )
                current = self._record_document2_pending_revision(
                    current, node, order, shell, candidate, revision, legacy_patch
                )
            except WorkflowContractError as exc:
                current = self._mark_agent_dispatch(
                    current,
        ...
        def cache_expectation_detail_outcome(outcome: _ParallelAgentOutcome) -> None:
            ...
            if self._is_expectation_detail_model_timeout_result(outcome.result):
                ...
                return
            retry_attempt = (
                1 if isinstance(outcome.job.extra_context.get("detail_recovery_retry"), dict) else 0
            )
            accept_detail_result(
                order, shell, outcome.result, cached=False, retry_attempt=retry_attempt
            )

        outcomes_by_order = {
            outcome.job.order: outcome
            for outcome in self._run_agent_jobs_concurrently(
                current, node, jobs, on_outcome=cache_expectation_detail_outcome
            )
        }
        ...
        for order, timeout_error in list(timed_out_orders.items()):
            ...
            current = self._prepare_expectation_detail_timeout_retry(
                current, node, order, shells[order], timeout_error
            )
    ...
    def _expectation_detail_context(
        self, shell: ExpectationShell, *, recovery_error: str | None = None
    ) -> dict[str, Any]:
        ...
        budget: dict[str, Any] = {
            ...
            "evidence_gap_policy": "Record unknowns and rationale when the bounded narrative lookup cannot supply enough coverage; do not retry low-value tool calls.",
        }
        ...
        if recovery_error:
            context["detail_instruction"] = (
                "Recovery retry after a timed-out expectation detail worker. Use compact context only, avoid repeating low-value retrieval loops, and finish one schema-valid ExpectationDetailCandidateResult for this exact expectation shell. "
                + instruction
            )
            context["detail_completion_budget"] = budget | {
                "recovery_retry": True,
                "previous_timeout": recovery_error,
                "retry_policy": "At most one narrative-report attempt; if unavailable or already insufficient, finish from shell/context with explicit unknowns.",
            }
    ...
    def _is_expectation_detail_model_timeout_result(self, result: AgentResult) -> bool:
        ...
        return (
            isinstance(gateway_error, dict) and gateway_error.get("code") == "model_request_timeout"
        )
        ...
        *,
        ...
        retry_job = _ParallelAgentJob(
            ...
            extra_context=self._expectation_detail_context(
                shell, recovery_error=str(timeout_error)
            ),
        )
    ...
    def _record_expectation_detail_status(
        ...
        *,
        ...
        return checkpoint.model_copy(
            update={"metadata": checkpoint.metadata | {_EXPECTATION_DETAIL_STATUS_KEY: statuses}},
            deep=True,
        )
    ...
    def _validate_expectation_shells(
        self, ticker: str, result: AgentResult
    ) -> ExpectationShellConstructionResult:
        construction = self.output_validator.validate(
            result.payload, "ExpectationShellConstructionResult"
        )
    ...
    def _expectation_unit_candidate_from_detail_result(
        self, ticker: str, shell: ExpectationShell, result: AgentResult
    ) -> ExpectationUnitCandidate:
        candidate_result = self.output_validator.validate(
            result.payload, "ExpectationDetailCandidateResult"
        )
        ...
        if candidate_result.delegations:
            raise WorkflowContractError(
                "GenerateExpectationDetails detail candidates must not return delegations; record factual gaps in unknowns or rationale."
            )
    ...
    def _document2_revision_from_detail_candidate(
        self, candidate: ExpectationUnitCandidate
    ) -> Document2Revision:
    ...
    def _legacy_pending_patch_from_document2_revision(
        self, checkpoint: WorkflowCheckpoint, revision: Document2Revision
    ) -> BlackboardPatch:
        return BlackboardPatch(
            ...
            operation=PatchOperation.CREATE if revision.before is None else PatchOperation.UPDATE,
            ...
            rationale=f"Legacy pending patch derived programmatically from Document2Revision {revision.revision_id}.",
    ...
    def _record_document2_pending_revision(
        ...
        return checkpoint.model_copy(
            update={"metadata": checkpoint.metadata | {_DOCUMENT2_PENDING_REVISIONS_KEY: entries}},
            deep=True,
        )

    def _document2_pending_revision_entry(
        self, checkpoint: WorkflowCheckpoint, shell: ExpectationShell
    ) -> dict[str, Any] | None:
        raw = checkpoint.metadata.get(_DOCUMENT2_PENDING_REVISIONS_KEY)
        if not isinstance(raw, list):
            return None
    ...
    def _validate_expectation_detail_candidate_identity(
        self, ticker: str, shell: ExpectationShell, document: ExpectationUnitDocument
    ) -> None:
    ...
    def _validate_expectation_detail_quality(self, document: ExpectationUnitDocument) -> None:
        ...
        if not monitoring.positive_events or not monitoring.negative_events:
            raise WorkflowContractError(
                "GenerateExpectationDetails event_monitoring_direction needs positive and negative events."
            )
    ...
    def _review_expectation_fields(
        ...
        specs: list[dict[str, Any]] = [
            {
                ...
                "review_scope": ["expectation_name", "direction", "market_view", "realized_facts"],
                "instruction": "For A1, focus on DoxAtlas traceability, propositions, narrative source ids, ignored propositions, media/social capsules, and source support for market_view and realized_facts. If O1's statement is incomplete or misleading from DoxAtlas traceability, provide the corrected DoxAtlas-grounded formulation.",
                "tool_requirements": [
                    {
                        "tool_name": tool_name,
                        "required": False,
                        "purpose": "Optional low-level DoxAtlas source material for A1 audit.",
                    }
            ...
            },
            {
                ...
                "instruction": "For C1, focus on fundamentals, filings, financial statements, earnings events, press releases, company quality, valuation-sensitive facts, and current_state. If O1's statement is incomplete or misleading, provide the corrected fundamental formulation.",
            },
            {
                "agent_name": AgentName.C3_INDUSTRY_RESEARCH,
                "schema": "ExpectationFieldReviewResult",
                "content_type": "c3_industry_review",
                "review_scope": ["key_variables.current_state", "event_monitoring_direction"],
                "instruction": "For C3, focus on industry structure, peer context, sector trends, competitive position, policy, and thematic support. If O1's statement misses industry framing or peer-relative context, provide the corrected industry formulation.",
            },
            {
                ...
                "instruction": "For O4, focus on price reaction, OHLCV behavior, relative performance, volume, technical context, and market data. If O1 overstates or understates market pricing, provide the corrected market-action formulation.",
            },
        ]
        results: list[AgentResult] = []
        ...
        for order, spec in enumerate(specs):
                ...
                tool_requirements = [
                    {
                        "tool_name": tool_name,
                        "required": False,
                        "purpose": self._field_review_tool_purpose(agent_name, tool_name),
                    }
            ...
            pending_patch_context = self._field_review_pending_patch_context(
                agent_name, checkpoint.pending_patches
            )
            ...
            extra_context = {
                ...
                "review_instruction": "\n\n".join([review_common_instruction, spec["instruction"]]),
                "review_candidates": [
                    patch.after
                ...
                "review_context_compaction": {
                    ...
                    "reason": "ReviewExpectationFields uses compact role-specific patch and global-research context so reviewers focus on their field scope without replaying full expectation documents.",
                },
                "tool_requirements": tool_requirements,
                "required_tool_names": [],
                "react_runtime_budget": {"max_steps": 3, "max_tool_call_batches": 1},
            }
        ...
        for outcome in self._run_agent_jobs_concurrently(checkpoint, node, jobs):
            ...
            if result is None:
                ...
                continue
            accepted_result = self._accepted_document2_reviewer_result(
                checkpoint, node, result, expected_schema=spec["schema"]
            )
            if accepted_result is None:
                first_error = first_error or WorkflowContractError(
                    f"{node.value}/{outcome.job.agent_name.value} reviewer output was skipped by acceptance checks."
                )
                skipped_reviewer_count += 1
                continue
            result = accepted_result
        ...
        review_findings = self._bridge_document2_blocking_findings_to_objections(
            checkpoint, review_findings
        )
        review_state: dict[str, Any] = {
            "primary_state": DOCUMENT2_REVIEW_FINDINGS_KEY,
            "finding_count": len(review_findings),
            "legacy_objection_bridge_count": sum((len(result.objections) for result in results))
    ...
    def _mock_resolve_blockers(self, checkpoint: WorkflowCheckpoint) -> None:
        ...
        for delegation in self.blackboard.list_blocking_delegations(checkpoint.run_id):
            self.blackboard.complete_delegation(
                checkpoint.run_id, delegation.delegation_id, "Mock A2 fact-check completed."
            )
    ...
    def _validate_expectation_patch_list(
        self, ticker: str, expectation_patches: list[BlackboardPatch]
    ) -> None:
        expectation_patches = [
            patch
