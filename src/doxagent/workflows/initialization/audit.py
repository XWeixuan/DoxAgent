...
class InitializationAuditMixin:
    ...
    def _write_working_memory(
        self, checkpoint: WorkflowCheckpoint, result: AgentResult, content_type: str
    ) -> None:
        try:
            self.blackboard.add_working_memory_entry(
                ...
                payload={
                    ...
                    "market_evidence_snapshot": result.payload.get("market_evidence_snapshot", {}),
                    "acceptance_audit": self._acceptance_audit(
                        checkpoint, result, parse_status="ok", schema_status="ok", write_status="ok"
                    ),
            ...
            raise WorkflowContractError(
                f"write_failed: could not write working memory entry for {(checkpoint.next_node.value if checkpoint.next_node else 'unknown')} {result.agent_name.value}: {exc}"
            ) from exc
    ...
    def _write_agent_acceptance_failure(
        ...
        *,
        ...
        try:
            ...
            raise WorkflowContractError(
                f"write_failed: could not write {event_code} for {task.run_metadata.workflow_node}/{task.agent_name.value}: {exc}"
            ) from exc
    ...
    def _write_parallel_agent_acceptance_failure(
        ...
        *,
        ...
        failed = result.model_copy(
            update={
                ...
                "error": AgentError(
                    ...
                    message=message,
                    retryable=False,
                    details={"expected_schema": expected_schema, "workflow_node": node.value},
                ),
            },
            deep=True,
        )
        try:
            ...
            raise WorkflowContractError(
                f"write_failed: could not write {event_code} for {node.value}/{agent_name.value}: {exc}"
            ) from exc

    def _write_workflow_exception(
        self, checkpoint: WorkflowCheckpoint, node: WorkflowNode, exc: Exception
    ) -> str | None:
        try:
            self.blackboard.add_working_memory_entry(
                checkpoint.run_id,
    ...
    def _agent_metadata(self, node: WorkflowNode, results: list[AgentResult]) -> dict[str, Any]:
        return {
            ...
            "last_error_code": next(
                (result.error.code for result in results if result.error is not None), None
            ),
        }
    ...
    def _agent_result_summary(self, result: AgentResult) -> dict[str, Any]:
        return {
            ...
            "patch_ids": [patch.patch_id for patch in result.proposed_patches],
            "evidence_ids": [evidence.evidence_id for evidence in []],
