...
class InitializationAgentDispatchMixin:
    def _run_agent(
        ...
        *,
        ...
        permissions = self._effective_permissions(
            definition.runtime.to_permissions(), node, task_type, agent_name
        )
        input_context = self._task_input_context(
            checkpoint, node, agent_name, task_type, permissions
        )
        if extra_context:
        ...
        def build_task(
            *, retry_attempt: int = 0, previous_failure: AgentResult | None = None
        ) -> AgentTask:
            task_input_context = input_context
            if retry_attempt:
                task_input_context = input_context | {
                    "retry_context": self._agent_retry_context(
                        previous_failure, retry_attempt=retry_attempt
                    )
                }
            return AgentTask(
                task_id=new_id("task"),
        ...
        def run_task(task: AgentTask) -> AgentResult:
            ...
            workflow_normalizes_document3 = (
                output_schema in _DOCUMENT3_WORKFLOW_NORMALIZED_OUTPUT_SCHEMAS
            )
            ...
            if (
                ...
                and (not workflow_normalizes_document3)
            ):
                try:
        ...
        result = self._run_serial_agent_task(
            checkpoint, node, task, run_task, workflow_watchdog=workflow_watchdog
        )
        ...
        for job in pending_by_order.values():
            outcome = _ParallelAgentOutcome(
                ...
                error=WorkflowContractError(
                    f"parallel_agent_timeout: {self._parallel_job_label(node, job)} did not return within {timeout_seconds:g} seconds."
                ),
            )
            if on_outcome is not None:
                on_outcome(outcome)
    ...
    def _run_parallel_agent_job_once(
        self, checkpoint: WorkflowCheckpoint, node: WorkflowNode, job: _ParallelAgentJob
    ) -> AgentResult:
    ...
    def _run_serial_agent_task(
        ...
        *,
        ...
        running = self._serial_agent_dispatch_checkpoint(
            checkpoint, node, task, status="running", timeout_seconds=timeout_seconds
        )
        self._save_parallel_outcome_checkpoint(running)
        result = self._run_agent_task_with_workflow_timeout(
            task, run_task, timeout_seconds=timeout_seconds
        )
        ...
        try:
            ...
            return AgentResult(
                ...
                error=AgentError(
                    ...
                    message=f"{task.run_metadata.workflow_node}/{task.agent_name.value} exceeded workflow timeout {timeout_seconds:g} seconds.",
                    retryable=False,
                    details={
    ...
    def _agent_task_input_context_stats(self, task: AgentTask) -> dict[str, int]:
        rendered = json.dumps(
            task.input_context, ensure_ascii=True, default=str, separators=(",", ":")
        )
    ...
    def _effective_permissions(
        ...
        if node is WorkflowNode.GENERATE_GLOBAL_NARRATIVE_REPORT:
            ...
            updates["writable_targets"] = [DocumentType.GLOBAL_RESEARCH.value]
        if agent_name is AgentName.O1_EXPECTATION_OWNER and node in {
            WorkflowNode.GENERATE_EXPECTATION_DETAILS,
            WorkflowNode.RESOLVE_EXPECTATION_CONSTRUCTION,
            WorkflowNode.RESOLVE_OBJECTIONS_AND_DELEGATIONS,
        }:
    ...
    def _agent_retry_context(
        self, previous_failure: AgentResult | None, *, retry_attempt: int
    ) -> dict[str, Any]:
        return {
    ...
    def _merge_prefetched_tool_result(
        self, result: AgentResult, tool_result: ToolResult
    ) -> AgentResult:
        summary = ToolCallSummary(
            ...
            output_summary=self._tool_output_summary_text(
                tool_result.tool_name, tool_result.output_summary
            ),
        )
        merged_result = result.model_copy(
            update={"tool_calls": result.tool_calls + [summary]}, deep=True
        )
    ...
    def _payload_mentions_narrative_gap(self, result: AgentResult) -> bool:
        ...
        return bool(
            ...
            and any(
                (
                    marker in text
                    for marker in ("missing", "failed", "gap", "unavailable", "缺失", "失败")
                )
            )
        )
