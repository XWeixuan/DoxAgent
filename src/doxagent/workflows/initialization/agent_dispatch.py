...
class InitializationAgentDispatchMixin:
    ...
    def _run_agent(self, checkpoint: WorkflowCheckpoint, node: WorkflowNode, agent_name: AgentName, task_type: TaskType, output_schema: str, *, extra_context: dict[str, Any] | None=None, audit_failures: bool=True, validate_output: bool=True, retry_on_retryable_failure: bool=True, workflow_watchdog: bool=True) -> AgentResult:
        definition = self.registry.get(agent_name)
        permissions = self._effective_permissions(definition.runtime.to_permissions(), node, task_type, agent_name)
        ...
        def build_task(*, retry_attempt: int=0, previous_failure: AgentResult | None=None) -> AgentTask:
            ...
            if retry_attempt:
                task_input_context = input_context | {'retry_context': self._agent_retry_context(previous_failure, retry_attempt=retry_attempt)}
            return AgentTask(task_id=new_id('task'), ticker=checkpoint.ticker, agent_name=agent_name, task_type=task_type, input_context=task_input_context, required_output_schema=output_schema, permissions=permissions, run_metadata=RunMetadata(run_id=checkpoint.run_id, ticker=checkpoint.ticker, workflow_node=node.value, created_at=datetime.now(UTC)))
        ...
        def run_task(task: AgentTask) -> AgentResult:
            ...
            return result
        task = build_task()
        result = self._run_serial_agent_task(checkpoint, node, task, run_task, workflow_watchdog=workflow_watchdog)
        if retry_on_retryable_failure and self._is_retryable_agent_result_failure(result):
            ...
            result = self._with_retry_audit(self._run_serial_agent_task(checkpoint, node, task, run_task, workflow_watchdog=workflow_watchdog), first_failure)
    ...
    def _run_agent_jobs_concurrently(self, checkpoint: WorkflowCheckpoint, node: WorkflowNode, jobs: list[_ParallelAgentJob], *, on_outcome: Callable[[_ParallelAgentOutcome], None] | None=None, timeout_seconds: float | None=None) -> list[_ParallelAgentOutcome]:
        ...
        timeout_seconds = float(timeout_seconds if timeout_seconds is not None else self.settings.workflow_agent_stale_after_seconds)
        deadline = time.monotonic() + timeout_seconds

        def worker(job: _ParallelAgentJob) -> None:
            outcome_queue.put(run_job(job))
        for job in jobs:
            thread = threading.Thread(target=worker, args=(job,), name=f'doxagent-{node.value}-{job.agent_name.value}-{job.order}', daemon=True)
            thread.start()
        ...
        for job in pending_by_order.values():
            outcome = _ParallelAgentOutcome(job=job, error=WorkflowContractError(f'parallel_agent_timeout: {self._parallel_job_label(node, job)} did not return within {timeout_seconds:g} seconds.'))
            if on_outcome is not None:
                on_outcome(outcome)
            outcomes.append(outcome)
    ...
    def _run_parallel_agent_job_once(self, checkpoint: WorkflowCheckpoint, node: WorkflowNode, job: _ParallelAgentJob) -> AgentResult:
        worker_checkpoint = checkpoint.model_copy(deep=True)
        return self._run_agent(worker_checkpoint, node, job.agent_name, job.task_type, job.output_schema, extra_context=deepcopy(job.extra_context), audit_failures=False, validate_output=False, retry_on_retryable_failure=False, workflow_watchdog=False)

    def _run_serial_agent_task(self, checkpoint: WorkflowCheckpoint, node: WorkflowNode, task: AgentTask, run_task: Callable[[AgentTask], AgentResult], *, workflow_watchdog: bool) -> AgentResult:
        if not workflow_watchdog:
            return run_task(task)
        timeout_seconds = self._serial_agent_timeout_seconds(node, task)
        running = self._serial_agent_dispatch_checkpoint(checkpoint, node, task, status='running', timeout_seconds=timeout_seconds)
        self._save_parallel_outcome_checkpoint(running)
        result = self._run_agent_task_with_workflow_timeout(task, run_task, timeout_seconds=timeout_seconds)
        if result.status is ResultStatus.FAILED and result.error is not None:
            failed = self._serial_agent_dispatch_checkpoint(running, node, task, status='failed', timeout_seconds=timeout_seconds, error_code=result.error.code, error_message=result.error.message)
            self._save_parallel_outcome_checkpoint(failed)
        return result

    def _run_agent_task_with_workflow_timeout(self, task: AgentTask, run_task: Callable[[AgentTask], AgentResult], *, timeout_seconds: float) -> AgentResult:
        ...
        thread = threading.Thread(target=worker, name=f'doxagent-serial-{task.run_metadata.workflow_node}-{task.agent_name.value}', daemon=True)
        thread.start()
        try:
            kind, value = outcome_queue.get(timeout=max(0.001, timeout_seconds))
        except Empty:
            return AgentResult(task_id=task.task_id, agent_name=task.agent_name, status=ResultStatus.FAILED, error=AgentError(code='workflow_agent_timeout', message=f'{task.run_metadata.workflow_node}/{task.agent_name.value} exceeded workflow timeout {timeout_seconds:g} seconds.', retryable=False, details={'workflow_node': task.run_metadata.workflow_node, 'agent_name': task.agent_name.value, 'task_type': task.task_type.value, 'required_output_schema': task.required_output_schema, 'timeout_seconds': timeout_seconds}))
        if kind == 'error':
            raise cast(BaseException, value)
        return cast(AgentResult, value)

    def _serial_agent_timeout_seconds(self, node: WorkflowNode, task: AgentTask) -> float:
        if node is WorkflowNode.RESOLVE_OBJECTIONS_AND_DELEGATIONS and task.agent_name is AgentName.O1_EXPECTATION_OWNER:
    ...
    def _serial_agent_dispatch_checkpoint(self, checkpoint: WorkflowCheckpoint, node: WorkflowNode, task: AgentTask, *, status: Literal['running', 'failed'], timeout_seconds: float, error_code: str | None=None, error_message: str | None=None) -> WorkflowCheckpoint:
        dispatch = {'run_id': checkpoint.run_id, 'workflow_node': node.value, 'agent_name': task.agent_name.value, 'task_id': task.task_id, 'task_type': task.task_type.value, 'required_output_schema': task.required_output_schema, 'timeout_seconds': timeout_seconds, 'status': status, 'updated_at': datetime.now(UTC).isoformat(), 'input_context_stats': self._agent_task_input_context_stats(task)}
        if error_code is not None:
            dispatch['error_code'] = error_code
        if error_message is not None:
            dispatch['error_message'] = error_message
        return checkpoint.model_copy(update={'metadata': checkpoint.metadata | {_SERIAL_AGENT_DISPATCH_KEY: dispatch}}, deep=True)

    def _agent_task_input_context_stats(self, task: AgentTask) -> dict[str, int]:
        rendered = json.dumps(task.input_context, ensure_ascii=True, default=str, separators=(',', ':'))
        return {'char_count': len(rendered), 'token_estimate': max(1, len(rendered) // 4)}

    def _effective_permissions(self, permissions: AgentPermissions, node: WorkflowNode, task_type: TaskType, agent_name: AgentName) -> AgentPermissions:
        ...
        if node is WorkflowNode.BUILD_GLOBAL_RESEARCH:
            ...
            if permissions.allowed_tools and task_type is TaskType.GENERATE_GLOBAL_RESEARCH and (agent_name is AgentName.O4_MARKET_TRACE):
        ...
        if node is WorkflowNode.GENERATE_GLOBAL_NARRATIVE_REPORT:
            updates['readable_context_scopes'] = [DocumentType.GLOBAL_RESEARCH.value, DocumentType.EXPECTATION_UNIT.value]
            updates['writable_targets'] = [DocumentType.GLOBAL_RESEARCH.value]
        if agent_name is AgentName.O1_EXPECTATION_OWNER and node in {WorkflowNode.GENERATE_EXPECTATION_DETAILS, WorkflowNode.RESOLVE_EXPECTATION_CONSTRUCTION, WorkflowNode.RESOLVE_OBJECTIONS_AND_DELEGATIONS}:
        ...
        if task_type is TaskType.GENERATE_EXPECTATION_UNIT:
            ...
            updates['writable_targets'] = [DocumentType.MONITORING_CONFIG.value]
        elif task_type in {TaskType.GENERATE_MONITORING_POLICY, TaskType.RESOLVE_MONITORING_POLICY}:
            updates['writable_targets'] = [DocumentType.MONITORING_POLICY.value]
        elif task_type in {TaskType.REVIEW_MONITORING_CONFIG, TaskType.REVIEW_MONITORING_POLICY}:
            updates['writable_targets'] = []
        elif node in {WorkflowNode.RESOLVE_EXPECTATION_CONSTRUCTION, WorkflowNode.RESOLVE_OBJECTIONS_AND_DELEGATIONS} and task_type is TaskType.REVIEW_EXPECTATION_FIELD:
    ...
    def _a1_allowed_tools_for_node(self, node: WorkflowNode) -> list[str]:
        return NODE_AGENT_ALLOWED_TOOL_OVERRIDES.get((node, AgentName.A1_DOXATLAS_AUDIT), [])
    ...
    def _agent_retry_context(self, previous_failure: AgentResult | None, *, retry_attempt: int) -> dict[str, Any]:
        return {'retry_attempt': retry_attempt, 'retry_reason': 'previous_agent_result_retryable_failure', 'previous_failure': self._agent_failure_audit(previous_failure)}
    ...
    def _merge_prefetched_tool_result(self, result: AgentResult, tool_result: ToolResult) -> AgentResult:
        summary = ToolCallSummary(tool_name=tool_result.tool_name, status=tool_result.status, input_summary='workflow 预取请求', output_summary=self._tool_output_summary_text(tool_result.tool_name, tool_result.output_summary))
        merged_result = result.model_copy(update={'tool_calls': result.tool_calls + [summary]}, deep=True)
    ...
    def _has_successful_tool_call(self, result: AgentResult, tool_name: str) -> bool:
        return any((tool_call.tool_name == tool_name and tool_call.status is ResultStatus.SUCCEEDED for tool_call in result.tool_calls))
    ...
    def _payload_mentions_narrative_gap(self, result: AgentResult) -> bool:
        ...
        text = ' '.join([*(item for item in unknowns if isinstance(item, str)), rationale if isinstance(rationale, str) else '']).lower()
        return bool(('doxatlas' in text or 'narrative' in text) and any((marker in text for marker in ('missing', 'failed', 'gap', 'unavailable', '缺失', '失败'))))
