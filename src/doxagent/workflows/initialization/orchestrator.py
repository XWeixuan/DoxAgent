...
class BlackboardInitializationWorkflow(
    ...
    def _default_runner(self) -> AgentRunner:
        if self.execution_mode == "agent_runner":
            return default_real_agent_runner(
                ...
                workflow_memory_compiler=WorkflowMemoryCompiler.from_repository(
                    self.blackboard.repository
                ),
            )
        return MockAgentRunner(
            self.registry, result_factory=InitializationMockResultFactory(include_blockers=True)
        )
    ...
    def resume(
        self, checkpoint: WorkflowCheckpoint, *, stop_after: WorkflowNode | None = None
    ) -> WorkflowExecutionResult:
        resumed = checkpoint
    ...
    def resume_latest(
        self, run_id: str, *, stop_after: WorkflowNode | None = None
    ) -> WorkflowExecutionResult:
        return self.resume(self.checkpoint_repository.get_latest(run_id), stop_after=stop_after)

    def _execute(
        self, checkpoint: WorkflowCheckpoint, *, stop_after: WorkflowNode | None
    ) -> WorkflowExecutionResult:
        ...
        try:
            ...
            blocked = failed_current.model_copy(
                update={
                    ...
                    | {"last_error_code": exc.__class__.__name__, "last_error_message": str(exc)},
    ...
    def _execute_node(
        ...
        if node == WorkflowNode.GENERATE_EXPECTATION_UNITS:
            return self._execute_node(
                checkpoint.model_copy(
                    update={"next_node": WorkflowNode.GENERATE_EXPECTATION_CONSTRUCTION}, deep=True
                ),
                WorkflowNode.GENERATE_EXPECTATION_CONSTRUCTION,
            )
        ...
        if node == WorkflowNode.GENERATE_GLOBAL_NARRATIVE_REPORT:
            self._require_documents(
                checkpoint, [DocumentType.GLOBAL_RESEARCH, DocumentType.EXPECTATION_UNIT]
            )
            result = self._run_agent(
                checkpoint,
                node,
                ...
                "ResearchSection",
                extra_context={
                    "section_instruction": "Summarize the overall market narrative structure after all expectation units have been promoted into belief state.",
    ...
    def _submit_result_patches(
        self, checkpoint: WorkflowCheckpoint, node: WorkflowNode, result: AgentResult
    ) -> WorkflowCheckpoint:
        ...
        return self._mark_completed(
            checkpoint, node, stable_document_types=stable_documents, metadata=metadata
        )
        ...
        for patch in result.proposed_patches:
            ...
            if patch.target.document_type not in {
                ...
                raise WorkflowContractError(
                    f"{node.value} produced unexpected document type: {patch.target.document_type.value}"
                )
    ...
    def _apply_latest_monitoring_config_runtime_state(
        ...
        for entry in bucket.values():
            ...
            try:
                ...
                self._record_monitoring_config_runtime_apply_audit(
                    checkpoint, document_id=document.document_id, audit=audit
                )
                return audit
        ...
        self._record_monitoring_config_runtime_apply_audit(
            checkpoint, document_id="missing", audit=audit
        )
        return audit

    def _apply_monitoring_config_patch(
        self, checkpoint: WorkflowCheckpoint, patch: BlackboardPatch
        ...
        for item in document.monitoring_items:
            tool_input, sanitize_audit = self._monitoring_runtime_apply_tool_input(
                checkpoint, item, raw_item=raw_items.get(item.item_id)
            )
    ...
    def _monitoring_runtime_apply_tool_input(
        ...
        *,
        ...
        raw_tool_input = (
            dict(raw_item.get("tool_input")) if isinstance(raw_item.get("tool_input"), dict) else {}
        )
        raw_source_id = str(
            raw_tool_input.get("source_id") or raw_item.get("source_id") or ""
        ).strip()
    ...
    def _monitoring_dropped_tool_input_fields(
        self, tool_input: dict[str, Any], source_id: str | None
    ) -> list[str]:
    ...
    def _monitoring_config_item_skip_audit(
        self, *, item_id: str, source_id: Any, reason: str, dropped_fields: Any = None
    ) -> dict[str, Any]:
        return {
            "item_id": item_id,
            "source_id": str(source_id or "missing"),
            "reason": self._compact_monitoring_apply_text(reason, limit=240),
            "required_fix": "revise tool_input to match monitoring.update_ticker_config source contract.",
            "dropped_fields": sorted((str(field) for field in dropped_fields or [])),
        }

    def _monitoring_config_apply_status(
        self, *, applied_count: int, skipped_count: int, total_count: int
    ) -> str:
    ...
    def _record_monitoring_config_runtime_apply_audit(
        self, checkpoint: WorkflowCheckpoint, *, document_id: str, audit: dict[str, Any]
    ) -> None:
    ...
    def _create_monitoring_config_apply_objection(
        ...
        *,
        ...
        required_fix = self._compact_monitoring_apply_text(
            "revise tool_input to match monitoring.update_ticker_config source contract.", limit=160
        )
        self.blackboard.create_objection(
            ...
            Objection(
                ...
                reason=f"MonitoringConfig item could not be applied to runtime Message Bus.\nitem_id: {item_id}\nsource_id: {source_id or 'missing'}\nreason: {compact_reason}\nrequired_fix: {required_fix}",
    ...
    def _review_monitoring_config(
        ...
        return self._run_document3_review_jobs(
            checkpoint, node, patch, specs, metadata_key="monitoring_config_review"
        )
    ...
    def _review_monitoring_policy(
        ...
        specs = [
            {
                ...
                "instruction": "Review whether O4 Monitoring Execution Policy can actually be triggered by the promoted Monitoring Config, whether it misclassifies cache-only messages as direct_trade, and whether every policy has supportable scope, trigger, action, and risk_guard. Raise blocking objections for mismatches.",
                "extra_context": {
        ...
        return self._run_document3_review_jobs(
            checkpoint, node, patch, specs, metadata_key="monitoring_policy_review"
        )
    ...
    def _resolve_monitoring_policy(
        ...
        patch, results = self._resolve_document3_pending_patch(
            ...
            extra_context={"monitoring_config_brief": self._monitoring_config_brief(checkpoint)},
        )
        stable_documents = self._submit_document3_brief_state_patch(
            checkpoint,
            patch,
            trigger_reason="Monitoring Execution Policy reviewed and promoted to Document 3 Brief State.",
        )
        metadata = self._agent_metadata(node, results) if results else {}
        metadata["document3_lifecycle"] = {
    ...
    def _submit_document3_brief_state_patch(
        self, checkpoint: WorkflowCheckpoint, patch: BlackboardPatch, *, trigger_reason: str
    ) -> list[DocumentType]:
        remaining = self._document3_unresolved_objections(checkpoint, patch)
        if remaining:
            raise WorkflowContractError(
    ...
    def _document3_pending_patch(
        self, checkpoint: WorkflowCheckpoint, document_type: DocumentType, node: WorkflowNode
    ) -> BlackboardPatch:
        matches = [
            patch
    ...
    def _direct_document_from_result(
        ...
        *,
    ...
    ) -> KnownEventsDocument | MonitoringConfigDocument | MonitoringPolicyDocument | None:
        ...
        if node is WorkflowNode.GENERATE_KNOWN_EVENTS:
            return self._normalize_known_events_document(
                checkpoint, structured, result, warnings=warnings
            )
        if node in {
            WorkflowNode.GENERATE_MONITORING_CONFIG,
            WorkflowNode.RESOLVE_MONITORING_CONFIG,
        }:
            return self._normalize_monitoring_config_document(
                checkpoint.ticker, structured, warnings=warnings
            )
        if node in {
            WorkflowNode.GENERATE_MONITORING_POLICY,
            WorkflowNode.RESOLVE_MONITORING_POLICY,
        }:
            return self._normalize_monitoring_policy_document(
                checkpoint.ticker, structured, warnings=warnings
            )
        return None
    ...
    def _normalize_known_events_document(
        ...
        *,
        ...
        if not isinstance(raw_events, list):
            self._document3_warning(
                normalization_warnings,
                "field_fallback_applied",
                "events",
                "Expected a list of known events.",
            )
            raw_events = []
        ...
        for index, item in enumerate(raw_events):
            if not isinstance(item, dict):
                self._document3_warning(
                    ...
                    "record_isolated",
                    f"events[{index}]",
                    "Known event is not a JSON object.",
                )
                continue
            try:
                ...
                expectation_id = self._known_event_expectation_id(
                    checkpoint, item, description, context=known_event_context
                )
                event_time, event_window = self._normalize_known_event_time_and_window(
                ...
                event = KnownEvent(
                    ...
                    duplicate_detection_keys=self._duplicate_detection_keys(
                        item, description, expectation_id
                    ),
                    ...
                    or self._known_event_has_price_reaction(description),
                    is_known_old_news=bool(item.get("is_known_old_news"))
                    or (
                        event_time is not None
                        and self._known_event_is_old_news(event_time, created_at)
                    ),
                )
                events.append(event)
            except (TypeError, ValueError) as exc:
                self._document3_warning(
                    normalization_warnings, "record_isolated", f"events[{index}]", str(exc)
                )
        if not events:
            self._document3_warning(
                normalization_warnings,
                "minimal_fallback_document",
                "events",
                "No known event record could be normalized.",
            )
        ...
        try:
            ...
            self._document3_warning(
                normalization_warnings, "final_schema_fallback", "known_events", str(exc)
            )
    ...
    def _normalize_known_event_time_and_window(
        self, item: dict[str, Any], description: str, created_at: datetime
    ) -> tuple[datetime | None, str | None]:
        raw_time = item.get("event_time")
        date_hint = item.get("date") or item.get("event_date")
        explicit_window = (
            str(
                item.get("event_window") or item.get("window") or item.get("time_window") or ""
            ).strip()
            or None
        )
        ...
        if (
            event_window is None
            and raw_time_text
            and not re.fullmatch(r"20\d{2}-\d{2}-\d{2}(?:[T ][^ ]+)?", raw_time_text)
        ):
    ...
    def _duplicate_detection_keys(
        self, item: dict[str, Any], description: str, expectation_id: str | None
    ) -> list[str]:
        values = [
    ...
    def _known_event_time_is_run_timestamp(self, value: Any, created_at: datetime) -> bool:
        ...
        return delta_seconds <= 300 and (event_time.hour, event_time.minute, event_time.second) != (
            0,
            0,
            0,
        )
    ...
    def _known_event_match_score(self, document: ExpectationUnitDocument, description: str) -> int:
        ...
        if any((token in text for token in ("capex", "资本开支", "hyperscaler", "roi"))) and (
            ...
            score += 4
        if any(
            (token in text for token in ("dram", "nand", "合约价", "库存", "周期", "价格"))
        ) and ("cycle" in identity or "周期" in identity):
            score += 4
        if any(
            (
                token in text
    ...
    def _known_event_time_hint_precise(self, description: str) -> str | None:
        ...
        if match:
            ...
            return f"{year}-{int(month):02d}-{int(day):02d}"
        quarter_patterns = (
            "\\b([1-4])Q\\s*[' ]?(20\\d{2}|\\d{2})\\b",
            "\\bQ([1-4])\\s*[' ]?(20\\d{2}|\\d{2})\\b",
            "\\bQ([1-4])\\s*FY\\s*(20\\d{2}|\\d{2})\\b",
        )
        ...
        year_quarter_patterns = (
            "\\b(20\\d{2})\\s*Q([1-4])\\b",
            "\\b(20\\d{2})\\s*年?\\s*Q([1-4])\\b",
            "\\bFY\\s*(20\\d{2}|\\d{2})\\s*Q([1-4])\\b",
        )
    ...
    def _normalize_monitoring_config_document(
        self, ticker: str, payload: dict[str, Any], *, warnings: list[dict[str, str]] | None = None
    ) -> MonitoringConfigDocument:
        normalization_warnings = warnings if warnings is not None else []
        raw_items = payload.get("monitoring_items") or payload.get("items") or []
        if not isinstance(raw_items, list):
            self._document3_warning(
                ...
                "field_fallback_applied",
                "monitoring_items",
                "Expected a list of monitoring items.",
            )
            raw_items = []
            ...
            try:
                if isinstance(item, dict):
                    ...
                    trigger_condition = (
                        ...
                        or f"Monitor {ticker}-specific messages."
                    )
                    ...
                    if not item_id:
                        ...
                        self._document3_warning(
                            normalization_warnings,
                            "field_fallback_applied",
                            f"monitoring_items[{index}].item_id",
                            f"Assigned {item_id}.",
                        )
                    items.append(
                        MonitoringItem(
                            ...
                            base_keywords=self._string_list(
                                item.get("base_keywords"), fallback=name
                            ),
                            ...
                            expectation_id=str(item.get("expectation_id"))
                    ...
                    self._document3_warning(
                        normalization_warnings,
                        "field_fallback_applied",
                        f"monitoring_items[{index}]",
                        "Converted text item to a safe monitoring record.",
                    )
                    items.append(
                        MonitoringItem(
                            item_id=f"mi_{index + 1:03d}",
        ...
        if not items:
            self._document3_warning(
                ...
                "minimal_fallback_document",
                "monitoring_items",
                "No monitoring item could be normalized.",
            )
            items = [self._fallback_monitoring_item(ticker)]
        created_at = self._coerce_event_time(payload.get("created_at"))
        try:
            candidate = MonitoringConfigDocument(
                document_id=str(payload.get("document_id") or new_id("doc")),
                ticker=ticker,
                created_at=created_at,
            ...
            self._document3_warning(
                normalization_warnings, "final_schema_fallback", "monitoring_config", str(exc)
            )
            fallback = MonitoringConfigDocument(
                document_id=new_id("doc"),
                ticker=ticker,
                created_at=created_at,
                monitoring_items=[self._fallback_monitoring_item(ticker)],
            )
    ...
    def _fallback_monitoring_item(self, ticker: str) -> MonitoringItem:
        ...
        return MonitoringItem(
            ...
            base_keywords=[ticker],
            priority="low",
            trigger_condition=f"Keep {ticker} monitoring disabled until a valid configuration is reviewed.",
        )

    def _monitoring_tool_input(
        self, *, checkpoint_ticker: str, item: dict[str, Any]
    ) -> dict[str, Any]:
        raw_tool_input = dict(item.get("tool_input") or {})
        source_id = (
            str(raw_tool_input.get("source_id") or item.get("source_id") or "stocktwits_messages")
            .strip()
            .lower()
        )
        tool_input: dict[str, Any] = {
        ...
        for field, max_items in parameter_schema_for_source(source_id).items():
            values = self._dedupe_texts(
                [*self._string_list(raw_tool_input.get(field)), *self._string_list(item.get(field))]
            )
            if values:
                tool_input[field] = values[:max_items]
        return tool_input
    ...
    def _normalize_monitoring_policy_document(
        ...
        normalization_warnings = warnings if warnings is not None else []
        candidates = [
            *self._normalize_policy_rules_safe(
            ...
            *self._normalize_policy_rules_safe(
                ...
                default_action_type=PolicyActionType.PUSH_TO_AGENT,
                ticker=ticker,
                field_name="push_to_agent_rules",
                warnings=normalization_warnings,
            ),
        ]
        ...
        if not policies:
            self._document3_warning(
                ...
                "minimal_fallback_document",
                "policies",
                "No monitoring policy rule could be normalized.",
            )
            policies = [self._fallback_monitoring_policy_rule(ticker)]
        direct = [
            rule for rule in policies if rule.policy_type == PolicyActionType.DIRECT_TRADE.value
        ]
        push = [rule for rule in policies if rule.policy_type == "escalate"]
        no_action_rationale = (
            str(
                payload.get("no_action_rationale") or payload.get("omission_rationale") or ""
            ).strip()
            or None
        )
        missing_paths = [
            name for name, rules in (("direct_trade", direct), ("escalate", push)) if not rules
        ]
        ...
        created_at = self._coerce_event_time(payload.get("created_at"))
        try:
            candidate = MonitoringPolicyDocument(
                document_id=str(payload.get("document_id") or new_id("doc")),
                ticker=ticker,
                created_at=created_at,
                policies=policies,
            ...
            self._document3_warning(
                normalization_warnings, "final_schema_fallback", "monitoring_policy", str(exc)
            )
            fallback_rule = self._fallback_monitoring_policy_rule(ticker)
            fallback = MonitoringPolicyDocument(
                document_id=new_id("doc"),
                ticker=ticker,
                created_at=created_at,
                policies=[fallback_rule],
                direct_trade_rules=[],
                push_to_agent_rules=[fallback_rule],
                cache_rules=[],
                no_action_rationale="No direct-trade rule was safe to normalize.",
            )
            return MonitoringPolicyDocument.model_validate(fallback.model_dump(mode="python"))

    def _normalize_policy_rules_safe(
        self,
        value: Any,
        *,
        ...
        if not isinstance(value, list):
            self._document3_warning(
                warnings, "field_fallback_applied", field_name, "Expected a list of policy rules."
            )
            return []
        ...
        for index, item in enumerate(value):
            ...
            if not isinstance(item, dict):
                self._document3_warning(
                    warnings, "record_isolated", location, "Policy rule is not a JSON object."
                )
                continue
            try:
                ...
                try:
                    action_type = (
                        PolicyActionType(raw_action_type)
                        if raw_action_type
                    ...
                    self._document3_warning(
                        warnings,
                        "field_fallback_applied",
                        f"{location}.action_type",
                        f"Normalized to {action_type.value}.",
                    )
                ...
                if policy_type not in {"direct_trade", "escalate"}:
                    ...
                    self._document3_warning(
                        warnings,
                        "field_fallback_applied",
                        f"{location}.policy_type",
                        f"Normalized to {policy_type}.",
                    )
                action_type = (
                    PolicyActionType.DIRECT_TRADE
                    if policy_type == "direct_trade"
                    else PolicyActionType.PUSH_TO_AGENT
                )
                policy_id = str(
                    item.get("policy_id")
                    or item.get("rule_id")
                    or item.get("id")
                    or f"policy_{index + 1:03d}"
                ).strip()
                rule_id = str(
                    item.get("rule_id") or item.get("id") or f"rule_{index + 1:03d}"
                ).strip()
                ...
                trigger_condition = str(
                    ...
                    or item.get("condition")
                    or item.get("description")
                    or trigger_value.get("condition")
                    or f"Escalate when a new {ticker}-specific message contradicts an active expectation and includes source evidence."
                ).strip()
                ...
                if policy_type == "direct_trade":
                    ...
                    action.setdefault(
                        "question",
                        f"Review whether this {ticker} signal changes an active expectation.",
                    )
                ...
                if not strategy_note:
                    strategy_note = self._policy_strategy_note_text(
                        item.get("rationale") or item.get("note"), action_type=action_type
                    )
                    self._document3_warning(
                        warnings,
                        "field_fallback_applied",
                        f"{location}.strategy_note",
                        "Applied deterministic runtime routing note.",
                    )
                rules.append(
                    MonitoringPolicyRule(
                        ...
                        reasoning=str(
                            ...
                            or "Policy routes Document3 runtime monitoring signals."
                        ),
                        evidence_fields=self._payload_string_list(item, "evidence_fields")
                        or self._payload_string_list(item, "required_evidence_fields"),
                        escalation_path=str(item.get("escalation_path") or item.get("route"))
                        if item.get("escalation_path") or item.get("route")
                        else None,
                    )
                )
            except (TypeError, ValueError) as exc:
    ...
    def _fallback_monitoring_policy_rule(self, ticker: str) -> MonitoringPolicyRule:
        ...
        return MonitoringPolicyRule(
            policy_id="policy_fallback_001",
            rule_id="rule_fallback_001",
            ...
            action={
                "send_to": ["O1", "O4"],
                "question": f"Review whether this {ticker} signal changes an active expectation.",
                "priority": "low",
            },
            risk_guard={"guardrail": "Do not create broker orders."},
            strategy_note="Route to research agents for evidence review; never execute a trade directly.",
            reasoning="Fallback preserves workflow continuity without authorizing an automated trade.",
        )
    ...
    def _policy_action_payload(self, value: Any, *, policy_type: str) -> dict[str, Any]:
        ...
        return {
            "send_to": ["O3"],
            "question": text or "Review runtime message routing and blackboard impact.",
            "priority": "medium",
        }
    ...
    def _document3_warning(
        self, warnings: list[dict[str, str]], code: str, location: str, message: str
    ) -> None:
        warnings.append(
            {
                "code": code,
                ...
                or "Document3 normalization warning.",
            }
        )

    def _validate_agent_success(
        self, result: AgentResult, node: WorkflowNode, *, require_patches: bool = True
    ) -> None:
    ...
    def _validate_policy_action_shape(self, rule: MonitoringPolicyRule) -> None:
        ...
        if missing:
            raise WorkflowContractError(
                f"GenerateMonitoringPolicy action is missing required fields for {rule.policy_type}: {', '.join(missing)}"
            )
    ...
    def _validate_o1_narrative_tool_gap(self, result: AgentResult, node: WorkflowNode) -> None:
        ...
        if self._payload_mentions_narrative_gap(result):
            return
        raise WorkflowContractError(
            f"{node.value} missed required doxa_get_narrative_report evidence without recording the DoxAtlas narrative gap in unknowns or rationale."
        )

    def _ensure_o1_narrative_tool_evidence(
        self, checkpoint: WorkflowCheckpoint, result: AgentResult, node: WorkflowNode
    ) -> AgentResult:
        if result.payload.get("runtime") != "react":
            return result
        tool_name = "doxa_get_narrative_report"
        ...
        if tool_registry is None:
            raise WorkflowContractError(
                f"tool_prefetch_failed: {node.value} requires {tool_name}, but the active runner has no tool registry."
            )
        ...
        if self._payload_mentions_narrative_gap(result):
            return result
        raise WorkflowContractError(
            f"tool_prefetch_failed: {node.value} required {tool_name}, but prefetch failed: {message}"
        )
    ...
    def _submit_patch(
        ...
        *,
        ...
        self.blackboard.submit_patch(
            run_id, patch, permissions=permissions, trigger_reason=trigger_reason
        )

    def _require_documents(
        self, checkpoint: WorkflowCheckpoint, required: list[DocumentType]
    ) -> None:
    ...
    def _resolve_research_inputs(
        self, ticker: str, research_inputs: GlobalResearchInputs | dict[str, Any] | None
    ...
    def _task_input_context(
        ...
        if history_blocks_needed:
            context["working_memory_summary"] = [
                ...
                for entry in repository.list_working_memory_summaries(
                    checkpoint.run_id, include_payload=False
                )
            ]
        ...
        if self._workflow_task_keeps_global_research_context(node):
            ...
            global_research_context = self._global_research_context_from_belief_state(
                run, node=node, agent_name=agent_name, task_type=task_type, permissions=permissions
            )
    ...
    def _workflow_document_bucket_run(
        self, checkpoint: WorkflowCheckpoint, document_types: list[DocumentType]
    ) -> BlackboardRun:
    ...
    def _summary(
        self, checkpoint: WorkflowCheckpoint, *, notes: list[str] | None = None
    ) -> WorkflowRunSummary:
    ...
    def _result(
        self, checkpoint: WorkflowCheckpoint, *, error: str | None = None
    ) -> WorkflowExecutionResult:
