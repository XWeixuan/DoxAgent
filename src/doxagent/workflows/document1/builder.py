...
class Document1BuilderMixin:
    def _build_global_research_with_agent_runner(
        ...
        specs = [
            (
                ...
                "fundamental_report",
                "Generate a sourced ResearchSection covering recent fundamental developments and how longer-cycle fundamentals explain current market attention.",
            ),
            (
                AgentName.C2_MACRO_RESEARCH,
                "macro_report",
                "Generate a sourced ResearchSection covering recent macro changes that affect current pricing, using longer-cycle macro context only when it explains the current setup.",
            ),
            (
                AgentName.C3_INDUSTRY_RESEARCH,
                "industry_report",
                "Generate a sourced ResearchSection covering recent industry and competitive developments, grounded in structural industry context where useful.",
            ),
            (
                AgentName.O4_MARKET_TRACE,
                "market_trace_report",
                "Generate a sourced ResearchSection covering recent price and flow reaction first, with broader chart history used only as baseline context.",
            ),
        ]
        ...
        for order, (agent_name, section_key, instruction) in enumerate(specs):
            ...
            current = self._mark_agent_dispatch(
                current, node, agent_name, status="running", section_key=section_key
            )
            jobs.append(
                _ParallelAgentJob(
                    order=order,
                    ...
                    extra_context=self._global_research_agent_context(
                        inputs, section_key=section_key, instruction=instruction
                    ),
                )
            )
        if jobs:
        ...
        def cache_global_research_outcome(outcome: _ParallelAgentOutcome) -> None:
            ...
            current = self._store_global_research_agent_result(
                current, node, outcome.job.agent_name, section_key, outcome.result
            )
            self._save_parallel_outcome_checkpoint(current)
        ...
        outcomes_by_order = {
            ...
            for outcome in self._run_agent_jobs_concurrently(
                current, node, jobs, on_outcome=cache_global_research_outcome
            )
        }
        ...
        for order, (agent_name, section_key, _instruction) in enumerate(specs):
            ...
            try:
                ...
                sections[section_key] = self._ensure_global_research_section_content(
                    current, section_key, section, result
                )
            ...
            if cached is None:
                current = self._store_global_research_agent_result(
                    current, node, agent_name, section_key, result
                )
                self.checkpoint_repository.save_checkpoint(current)
        ...
        self._submit_patch(
            current.run_id, patch, f"{node.value} 已由 C1/C2/C3/O4 汇总 GlobalResearchDocument。"
        )
        ...
        return self._mark_completed(
            ...
            | {
                "global_research_downstream_context": self.global_research_assembler.downstream_context(
                    results
                ),
    ...
    def _assert_no_document1_proposed_patches(
        self, result: AgentResult, node: WorkflowNode
    ) -> None:
        if result.proposed_patches:
            raise WorkflowContractError(
                f"{node.value} forbids proposed_patches: Document1 agents must return ResearchSection output; GlobalResearchDocument assembly is owned by the workflow transaction layer."
            )

    def _research_section_from_result(
        self, result: AgentResult, expected_schema: str
    ) -> ResearchSection:
        ...
        section = (
            model if isinstance(model, ResearchSection) else ResearchSection.model_validate(model)
        )
        return section

    def _global_research_patch(
        self, document: GlobalResearchDocument, results: list[AgentResult]
    ) -> BlackboardPatch:
