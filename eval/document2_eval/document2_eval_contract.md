...
```powershell
ssh doxagent-hk 'cd /root/doxagent && git pull --ff-only && docker compose build runtime-scheduler && docker compose up -d runtime-scheduler'
```
...
```powershell
ssh doxagent-hk 'cd /root/doxagent && docker compose run --rm -e DOXAGENT_RUN_REAL_API_TESTS=1 -e DOXAGENT_STORAGE_MODE=postgres runtime-scheduler python eval/run_document2_expectation_units_smoke.py <source_run_id> --mode clone --stop-after PromoteExpectationToBeliefState --export-brief-state'
```
...
3. 本地 `git push`，云端 `git pull --ff-only`，然后在 `/root/doxagent` 执行 `docker compose build runtime-scheduler`。
...
15. 再次 `git push`、云端 `git pull --ff-only`、`docker compose build runtime-scheduler`，用同一停止点和同一评分标准在云端复测。
