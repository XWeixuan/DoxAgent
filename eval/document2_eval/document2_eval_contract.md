...
9. 按 `document2_rubrics.yaml` 逐项评分，即使硬门槛失败也可用于诊断。
10. 把 baseline 追加到 `document2_eval_records.md`。
11. 聚类失败类别，写出以质量提升为中心的可验证优化假设。
12. 在修改代码、prompt、schema 或 workflow 前记录 baseline commit 或 dirty-tree 摘要。
13. 执行范围最小的修改，并追加 `changelog`。
14. 再次 `git push`、云端 `git pull --ff-only`、`docker compose build debug-viewer`，用同一停止点和同一评分标准在云端复测。
15. 追加 retest 结果，比较 hard gate、rubric delta 和 Document 2 质量变化。
16. 只有当目标质量问题改善且没有新增不可接受硬门槛失败时，才接受修改。
