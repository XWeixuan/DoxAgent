...
本文件用于追加记�?Document 2 预期单元 eval 循环。当前文件先提供模板，后续每一次可评估真实运行都应追加在本文件末尾�?
记录原则�?
- 每个可评�?run 都要记录，包�?partial、blocked、failed 或质量门槛失败的 run�?- baseline �?retest 必须写在同一�?eval section 下，便于比较�?- 不要�?pending patch 写成 stable expectation_unit�?- 不要�?Document 2 focused smoke 写成完整 Blackboard 初始化通过�?- 如果本轮只停�?`GenerateExpectationDetails`、`ReviewExpectationFields` �?`ResolveObjectionsAndDelegations`，必须在标题和结果里标注 partial scope�?
## Baseline 记录模板
...
- New quality blocker 4: stable market/variable fields still contain degraded placeholder wording, so uncertainty discipline is visible but not useful enough for a �?.2 core-quality target.
...
- Terminal error: `ReviewExpectationFields agent result failed: ReAct step 未返�?final payload、工具调用或委托。`
...
- category: `review_no_progress_terminal_failure`
  ...
  - evidence: terminal error `ReAct step 未返�?final payload、工具调用或委托。`; LangSmith `C1.ReviewExpectationFields.LOOP8` returned progress/ledger text instead of the action contract.
  - severity: blocking/workflow-runtime.
...
- Expected measurable improvement in the next cloud verification:
  - Retest no longer terminates at `ReviewExpectationFields agent result failed: ReAct step 未返�?final payload、工具调用或委托。`;
...
## Loop 1 Retest22 - built-in validators passed, but deterministic cleanup placeholders reached stable expectation units

### Run Metadata
- Date: 2026-06-24.
- Source run: `run_58f5afce8b9441ca804a2cde1ad9aec8` (Document 1-only source, unchanged).
- Execution run: `run_e4e5130167684dc2ab943ac52a9790d9`.
- Deployed commit: `826f083`.
- Remote cwd: `/root/doxagent`.
- Remote log: `.eval_runs/document2-loop1-retest22-20260624T082754+0800.log`.
- Log-reported Brief State export path: `eval/brief_state_exports/run_e4e5130167684dc2ab943ac52a9790d9.json`.
- Actual export-file status: missing on remote filesystem at evaluation time; Brief State and hard validators were rebuilt from Postgres using `DebugRunQueryService.brief_state(run_id)`.
- Cloud command: `docker compose -f docker-compose.yml run --rm -e DOXAGENT_RUN_REAL_API_TESTS=1 -e DOXAGENT_STORAGE_MODE=postgres debug-viewer python eval/run_document2_expectation_units_smoke.py run_58f5afce8b9441ca804a2cde1ad9aec8 --mode clone --stop-after PromoteExpectationToBeliefState --export-brief-state`.
- Polling discipline: no Codex automation or heartbeat automation; status checks used in-thread `Start-Sleep -Seconds 900`.

### Status
- Result: `completed_to_stop_after_but_quality_failed`.
- Smoke log final event: `document2_smoke_finished`, `status=running` by stop-after design, `next_node=GenerateGlobalNarrativeReport`.
- Completed target node: `PromoteExpectationToBeliefState`.
...
- Open objections: 0.
- Blocking delegations: 0.
- Improvement vs Retest21:
  - Retest21's resolver top-level partial-field loss did not recur.
  - O1 resolver and deterministic normalization produced promotable patches from a lifecycle perspective.
  - Built-in hard validators all passed for the rebuilt Brief State.
- Remaining quality failure:
  - Stable expectation units contain deterministic fallback text such as `Exact price reaction removed`, `rebuild the move from`, `Realized facts retain event direction`, and `Exact numeric thresholds were removed`.
  - These strings are safer than unsupported false precision, but they are not evidence-rich, auditable priced-in reasoning suitable for downstream monitoring/optimization.
  - The failure is not a runtime blocker; it is a promotion-quality gate gap.

### Built-in Hard Validators
| Validator | Result | Evidence | Notes |
| --- | --- | --- | --- |
| evidence_reference_integrity | pass | `checked_items=53`, `finding_count=0`. | Structural evidence refs are present and locatable. This does not prove semantic sufficiency. |
| langsmith_trajectory_tool_boundary | pass | `checked_items=53`, `finding_count=0`. | Workflow reached the stop-after target and trace boundary is closed enough for this focused smoke. |
| commit_log_state_mutation_consistency | pass | `checked_items=12`, `finding_count=0`. | Stable state mutations and commit log agree for promoted expectation units. |

### Hard Gate Failure Root Cause Matrix
| Gate | Result | failure_kind | Failure point | Root cause / fix target |
| --- | --- | --- | --- | --- |
| D2-HG01 | pass | none | Source handoff | Source remains the required Document 1-only run `run_58f5afce8b9441ca804a2cde1ad9aec8`. |
| D2-HG02 | pass | none | Stop-after path | `PromoteExpectationToBeliefState` completed; the following `running` status is expected because the smoke stops before the next node. |
| D2-HG03 | pass | none | Construction lifecycle | Construction, construction review/resolution, details, field review, resolver, and promotion all executed. |
| D2-HG04 | pass_with_caveat | quality_residual | Detail patches | Required fields exist and promoted, but several fields are fallback summaries rather than evidence-derived conclusions. |
| D2-HG05 | pass_with_caveat | semantic_evidence_gap | Evidence refs | Built-in ref validator passes, but price-reaction/fundamental cleanup text does not show a source-specific reconstruction from attached evidence. |
| D2-HG06 | fail | quality_residual | Price-in reasoning | Price reactions include placeholder language such as `Exact price reaction removed` and do not compute a target-ticker market-data baseline. |
| D2-HG07 | pass | none | Field review lifecycle | A1/C1/C3/O4 review pressure ran and produced useful objections. LangSmith shows some `Arrearage` retries but successful loops followed. |
| D2-HG08 | pass | none | Resolver lifecycle | O1 resolver/deterministic cleanup closed all blockers; no unresolved objections remained. |
| D2-HG09 | pass | none | Promotion lifecycle | Two stable expectation units were promoted with `pending_patch_count=0`. |
| D2-HG10 | pass | none | Trace/process closure | Built-in trajectory validator passed; DB and LangSmith can reconstruct the lifecycle. |
| D2-HG11 | pass_with_caveat | traceability_gap | Failure auditability | DB, remote log, hard validators, Working Memory, and LangSmith reproduce the run; the log-reported Brief State export file is still missing. |
| D2-HG12 | fail | context_value | Context/value control | Review loops still show high-token contexts and tool-plan failures (`Tavily` HTTP 432, some `Arrearage` retries), while deterministic cleanup produces low-value placeholders. |
| D2-HG13 | pass_with_caveat | memory_continuity_partial | Revision continuity | Objection resolution and promotion continuity worked structurally, but low-quality fallback content survived into stable memory. |

### Rubrics
| Rubric | Score | Reason |
| --- | ---: | --- |
| D2-R01 | 4 | Source discipline remains correct and the Document2 run starts from the required Document1-only source. |
| D2-R02 | 3 | Two stable theses are differentiated enough to monitor, but the fallback cleanup weakens thesis specificity and reviewability. |
| D2-R03 | 2 | Realized facts contain placeholders such as event-direction retention instead of stable, evidence-specific realized-event summaries. |
| D2-R04 | 1 | Priced-in reasoning is not acceptable: price reactions explicitly say exact reactions were removed and must be rebuilt from market evidence. |
| D2-R05 | 3 | Key variables exist and are tied to thesis direction, but unsupported numeric precision removal leaves several variables too generic for downstream use. |
| D2-R06 | 3 | Event monitoring directions are present and not empty, but some triggers are weakened by generic cleanup and source-review caveats. |
| D2-R07 | 3 | Evidence refs pass structurally and include market/filing evidence, but final text does not consistently bind claims to those refs. |
| D2-R08 | 4 | Field-review pressure is meaningful and caught real price/guidance/evidence issues; review did not silently pass weak claims. |
| D2-R09 | 3 | Resolver lifecycle now works structurally, but the accepted revisions degrade content into deterministic fallback text. |
| D2-R10 | 3 | Promotion readiness is structurally clean, yet the stable output is not quality-ready for monitoring because placeholder text survived. |
| D2-R11 | 3 | Tool/process behavior is usable with retries, but `Arrearage` and `Tavily` 432 failures show remaining fragility and wasted context. |
| D2-R12 | 2 | Uncertainty handling is too blunt: instead of carrying precise unresolved unknowns, the system emits broad placeholder cleanup text. |
| D2-R13 | 3 | DB, log, validators, and LangSmith allow reconstruction, but the missing Brief State export file weakens reproducibility. |
| D2-R14 | 5 | The optimization target is concrete: block deterministic placeholders at promotion and synthesize price reaction from structured OHLCV snapshots when available. |
...
- Quality target met: no.
- Operational improvement accepted: partial. Retest22 proves the resolver/merge lifecycle can reach stable promotion, but the promoted content is below strict Blackboard quality standards.

### Failure Categories
- category: `deterministic_numeric_cleanup_placeholder_quality`
  - issue: Deterministic cleanup removed unsupported precision but replaced it with generic placeholder language.
  - evidence: stable expectation text includes phrases such as `Exact numeric thresholds were removed`, `Realized facts retain event direction`, and `structured recalculation`.
  - severity: quality-blocking.
  - suspected root cause: cleanup fallback strings were designed to avoid false precision, not to remain useful as stable downstream monitoring inputs.
- category: `promotion_quality_gate_marker_gap`
  - issue: Existing promotion quality gate rejected some old placeholders but did not catch Retest22's newer cleanup strings.
  - evidence: built-in validators and promotion passed while stable text still contained `Exact price reaction removed` and related fallback markers.
  - severity: hard-gate quality gap.
  - suspected root cause: `_UNPROMOTABLE_EXPECTATION_TEXT_MARKERS` did not include the current deterministic cleanup vocabulary.
- category: `price_reaction_snapshot_not_rebuilt`
  - issue: Structured market evidence existed, but placeholder price reactions were not rebuilt into target-ticker OHLCV period-return baselines.
  - evidence: final price reaction says to rebuild from OHLCV/market-trace evidence instead of presenting a computed baseline.
  - severity: D2-HG06 failure.
  - suspected root cause: `_normalize_expectation_price_reaction_patch()` treated market snapshot presence as enough to attach refs, not as a signal to synthesize a concrete price reaction.
- category: `numeric_detection_over_triggers_fiscal_tokens`
  - issue: Numeric sanity cleanup risked treating fiscal/product tokens such as `Q3`, `FY2026`, or `HBM4` as unsupported numeric claims.
  - evidence: false positives increase the chance of fallback text replacing valid event semantics.
  - severity: quality-degrading.
  - suspected root cause: `_contains_numeric_value()` lacked non-claim numeric-token filtering.
- category: `brief_state_export_missing`
  - issue: smoke log reported an export path, but no JSON file existed under `eval/brief_state_exports` for the Retest22 run id.
  - evidence: hard validators had to be run from a Postgres-rebuilt Brief State.
  - severity: traceability caveat.
  - suspected root cause: smoke/export path handling may report intended path even when the final export file is not persisted.

### Optimization Hypothesis
- If the promotion quality gate treats the current deterministic cleanup vocabulary as unpromotable, then narrative-only fallback price/fact/variable text cannot enter stable expectation units.
- If price-reaction normalization uses attached structured OHLCV snapshots to synthesize a concrete target-ticker period-return baseline, then supported price-in reasoning can survive promotion without unsupported narrative precision.
- If structured market evidence is absent or lacks target-ticker OHLCV, the system should block promotion rather than promote a placeholder telling downstream users to rebuild the field.
- If fiscal-year, quarter, and product-generation tokens are ignored by numeric-claim detection unless they carry claim units/currency/percent, deterministic cleanup should preserve event semantics while still removing unsupported market/fundamental precision.
- Expected measurable improvement in the next cloud verification:
  - Stable expectation units should either contain concrete OHLCV-backed price reactions or fail before promotion with a specific quality error.
  - Retest22 phrases such as `Exact price reaction removed`, `structured recalculation`, and `Realized facts retain event direction` should not appear in stable belief state.
  - D2-R04, D2-R06, and D2-R12 should improve if structured evidence is available; otherwise D2-HG09 may intentionally fail as a stricter safety outcome.

### Proposed Modification Plan
- Change 1: Expand `_UNPROMOTABLE_EXPECTATION_TEXT_MARKERS` to include Retest22's observed cleanup placeholders and field-review fallback vocabulary.
- Change 2: Add non-claim numeric-token filtering so `Q3`, `FY2026`, `HBM4`, and standalone calendar years do not trigger numeric-sanity cleanup unless attached to claim units or market/fundamental quantities.
- Change 3: Update `_normalize_expectation_price_reaction_patch()` so placeholder/narrative-only price reactions with structured market evidence are rebuilt from target-ticker OHLCV snapshot metadata.
- Change 4: If target-ticker OHLCV is unavailable, emit an explicitly unpromotable price-reaction blocker text rather than a stable placeholder.
- Change 5: Remove or rewrite generic deterministic fallback strings that mention `structured recalculation`, event-direction retention, or source-rebuild instructions.
- Change 6: Add focused regression tests for narrative-only price-reaction rejection, OHLCV snapshot synthesis, non-claim numeric tokens, and the updated promotion-quality boundary.
- Retest requirement: push/deploy and launch another cloud smoke because Retest22 did not meet the quality target even though built-in validators passed.

### Actual Modification
- Implemented after this evaluation:
  - Updated `src/doxagent/workflows/initialization.py` to add Retest22 placeholder vocabulary to the unpromotable promotion gate.
  - Updated `_contains_numeric_value()` with non-claim token filtering for fiscal/product/year-like numbers while preserving true price, percentage, multiple, bps, and large-unit claims.
  - Updated price-reaction normalization to synthesize a concrete target-ticker OHLCV period-return baseline from structured market evidence when available.
  - Updated price-reaction fallback behavior so missing target-ticker OHLCV becomes an unpromotable blocker rather than a stable priced-in statement.
  - Removed the appended `Exact numeric thresholds were removed` note and rewrote deterministic fallback strings away from generic `structured recalculation` language.
  - Added/updated tests in `tests/test_phase5_initialization_workflow.py` for price-reaction placeholder blocking, OHLCV snapshot synthesis, non-claim numeric token handling, and stricter promotion rejection.
