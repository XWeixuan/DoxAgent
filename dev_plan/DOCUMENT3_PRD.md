...
DoxAgent 当前需要从“Blackboard 初始化系统”进一步扩展为围绕单个 ticker 持续运行的事件驱动研究与交易 agent 系统。新的整体结构不再以一次性生成研究报告为终点，而是形成一套围绕 ticker 的长期工作文档体系，支持后续持续监测、新旧消息识别、运行时决策、交易意图生成、后台 agent 推送、缓存处理以及未来交易复盘。
...
Trading Records 不在本轮 Document 3 开发范围内，但 Document 3 的 policy_id、trade intent、cache decision、background agent task 等输出需要为未来 Trading Records 预留可引用的结构。
...
* 新消息进入后应该触发 trade intent、background agent task，还是 cache decision？
* 哪些规则已经通过审查，可以被运行时使用？
...
* policy 是否把不该交易的缓存类消息误写成 direct trade
...
* 已知事件回顾
* 社媒重复转发
* 未提供新金额 / 数量 / 状态 / 时间节点的信息

Cache 不等于永久删除，而是进入缓存、忽略或批处理逻辑。
...
* cache_label 可标记为 old_duplicate、low_relevance、known_event_recap、weak_signal、background_only 等
* handling 用来说明进入缓存、忽略、批处理或仅记录
...
* 第一阶段不接实盘 broker
* 输出 trade intent、background agent task 或 cache decision
* policy_type 仅包含 direct_trade、escalate、cache
...
* cache action 包含 cache_label、handling
...
* trade intent / background agent task / cache decision 三类运行时输出
...
12. Policy 分为 direct_trade、escalate、cache 三类。
...
17. Cache 输出 cache decision。
18. Document 3 采用轻量 Brief State，不使用过重 promote 机制。
19. 所有关键变更进入 commit log，支持审计和回滚。
20. Trading Records 作为后置独立模块预留引用接口，但不在本轮实现。
