...
O3 暂时不启用。第一阶段不由 O3 生成交易策略，也不接入实盘 broker；交易相关运行规则先由 O4 在 Document 3 的 Monitoring Execution Policy 中以 trade intent / background agent task / cache decision 的形式表达。
...
* 定义新消息命中后应输出 trade intent、background agent task 还是 cache decision。
...
1. `direct_trade`：输出 trade intent，而不是真实订单；
2. `escalate`：输出 background agent task；
3. `cache`：输出 cache decision。
